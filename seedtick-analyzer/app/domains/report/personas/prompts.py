...
오직 너의 철학과 체크리스트 기준에 따라 구체적 팩트와 수치(PER, ROE, FCF, 마진율 등)를 직접 인용하여 날카롭게 평가하라.
안내 괄호 문구를 그대로 복사하지 말고, 반드시 실제 수치를 기반으로 너의 분석 내용을 직접 서술하라.

[공용 심층 데이터팩]
{datapack_markdown}

[작성 및 반환 형식 규칙]
반드시 아래 키워드와 형식을 정확히 지켜 작성하라. 인사말이나 서론은 일절 생략하라.
의견은 반드시 '매수', '보유', '관망', '매도' 중 하나를 선택하라.

인물: {persona_key} | 의견: 매수 | 확신도: 8
핵심 논거:
- ROE가 90% 이상이고 Forward PER 5배대로 저평가되어 강력한 안전마진 확보
- 영업이익률 48%로 메모리 사이클 상승기 고수익을 달성하며 잉여현금흐름 흑자 전환
적정가/매수 가격대: $210~$240
트리거·재검토 조건: HBM4 독점 공급 점유율 유지 여부, D램 ASP 하락 전환 시 재검토
대표 발언: "좋은 기업이 이 가격에 거래된다면 머뭇거릴 이유가 없다."
"""
